...
async def get_call_flow_diagnostics() -> CallFlowDiagnosticsResponse:
    """
    ...
    ASTERISK_MODE = settings.asterisk_mode
    ASTERISK_VPS_URL = settings.asterisk_vps_url or ""
    ASTERISK_AUDIOSOCKET_ENABLED = settings.asterisk_audiosocket_enabled
    ASTERISK_AUDIOSOCKET_HOST = settings.asterisk_audiosocket_host
    ASTERISK_AUDIOSOCKET_PORT = settings.asterisk_audiosocket_port
    ASTERISK_AUDIOSOCKET_TIMEOUT = settings.inactivity_timeout_seconds
    AUDIO_FORMAT = "slin16"
    ...
    vps_reachable, vps_latency = check_vps_connectivity(ASTERISK_VPS_URL) if ASTERISK_VPS_URL else (False, None)
    ...
    inbound_exists, inbound_audiosocket, inbound_correct = verify_inbound_dialplan() if ASTERISK_MODE == "vps" else (True, True, True)
    outbound_exists, outbound_audiosocket, outbound_correct = verify_outbound_dialplan() if ASTERISK_MODE == "vps" else (True, True, True)
    ...
    if not listening or (ASTERISK_MODE == "vps" and not vps_reachable):
        overall_status = "down"
...
async def health_check():
    ...
    return {
        ...
        "audiosocket_listening": check_audiosocket_listening(
            settings.asterisk_audiosocket_host, settings.asterisk_audiosocket_port
        )
    }
